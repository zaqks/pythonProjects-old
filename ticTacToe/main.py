...
from math_mdl import *
from perm_mdl import *
from time import sleep


print("press <Escape> to close the app")
...
class Win(Tk):
    ...
    def multi_unbind(self, n):
        a = 1
        for i in range(n):
            self.cnv.tag_unbind(str(a), print("yoo"))
            print("bind_set", a)
            a += 1

    def detect_touch(self, z):
        if Vb.can_play:
            if Vb.my_turn and not Vb.player_win and not Vb.computer_win:
                "convert to me"
                
                self.change_turn_lbl(2)
            ...
            elif not Vb.my_turn and not Vb.computer_win and not Vb.player_win:
                "convert to pc"
    ...
    def draw_x(self, x, y):
        k = Vb.a / Vb.n
        m = k/8
        self.cnv.create_line(x-(k/2)+m, y-(k/2)+m, x-(k/2)+k-m, y-(k/2)+k-m, fill="red", width=m, tags="cross")
        self.cnv.create_line(x-(k/2)+k-m,y-(k/2)+m, x-(k/2)+m, y-(k/2)+k-m, fill="red", width=m, tags="cross")

    def draw_o(self, x, y):
        k = Vb.a / Vb.n 
        r = k*6/16
        self.cnv.create_oval(x-r, y-r, x+r, y+r, fill=None, outline="blue", width = 15, tags="circle")
    ...
    def check_winner(self, p):
            ...
            for i in range(len(State.mlt_perm)):
                if State.mlt_perm[i] in Ref.mlt_combs:
                    ...
                    if cm in Ref.combs:
                        ...
                        self.cnv.create_line(p1[0], p1[1], p2[0], p2[1], fill="black", width=7.5, tags="line")
        ...
        if len(Vb.player_nums) + len(Vb.computer_nums) == Vb.n*Vb.n and not Vb.computer_win and not Vb.player_win:
            print("suiiii")
            self.win_lbl(3)
            Vb.can_play = False
    ...
    def change_turn_lbl(self, p):
        print("turn lbl")

        if p == 0:
            self.cnv.itemconfig("turn", text="", fill=Vb.p_c)
            self.cnv.update()

        if p == 1:
            self.cnv.itemconfig("turn", text=Vb.player+" turn", fill=Vb.p_c)
            self.cnv.update()

        if p == 2:
            self.cnv.itemconfig("turn", text=str(" ")+Vb.computer+" turn ", fill=Vb.c_c)
            self.cnv.update()
    ...
    def win_lbl(self, p):
        ...
        if p == 1:
            ...
            self.cnv.itemconfig("turn", text="No winner", fill="green")
            self.cnv.update()
    ...
    def reset(self, z):
        if not Vb.nopersn:
            ...
            Vb.can_play = False
            Vb.player_win = False
            Vb.computer_win = False

            Vb.player_nums.clear()
            Vb.computer_nums.clear()

            
            self.cnv.delete("cross")
            self.cnv.delete("circle")
            self.cnv.delete("line")

            self.multi_unbind(Vb.n*Vb.n)
            self.multi_bind(Vb.n*Vb.n)

            self.change_turn_lbl(0)
            
            self.create_welcome_win()

            print("X=" + str(Vb.x_vic) + " ; O=" + str(Vb.o_vic))
